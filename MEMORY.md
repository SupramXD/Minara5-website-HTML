...
## Verification copy (this variant of the site)
- This folder is the compliance/verification variant of the Studio Extrait site: original brand-free copy, no comparison pricing, published policy pages (about/terms/privacy), no construction badge, and the product detail page is served from `product.html`.
- Every published page carries `<meta name="description">`, canonical and OG tags, plus `robots.txt` and `sitemap.xml`. Crawler-facing files (MEMORY.md, AGENTS.md, tools/, input/, output/, functions/) are excluded in robots.txt.
- Ids for the three single fragrances are `profile-a-warm-day`, `profile-wild-bergamot` and `profile-the-bouquet`; the three single-fragrance ids use the `profile-` prefix and every `startsWith` guard accepts both the new and the older legacy prefix so cached data keeps rendering.
- The homepage feature copy, trust banner, footer description and policy text all come from Firestore (Site Texts / Returns policy) at runtime, so the admin panel must hold the same wording or it will overwrite the static defaults.
- Product naming (owner's rule, do not change): the fragrance's own theme stays in `nameShort` and is what the product title shows (`Caribbean`, `Wild Bergamot`, `The Bouquet`); the `name` field carries the "Inspired by <own descriptor>" line (`Inspired by A Warm Day`), and the UI label **`INSPIRED BY`** (cards, search, cart, reviews, product page badge) must stay - only the value after it is brand-free.
- **Honest Comparison accordion**: `product-core.js` rejects any stored text containing the brand placeholder, "designer", "similarity index" or "olfactory profile" and falls back to *"Blended in South Africa in small batches from premium imported oils at a dense 20%+ extrait concentration, then macerated for even, long wear. Expect 8-12 hours of longevity and strong projection, without the luxury brand premium."* It must never claim we author every scent signature ("our own formulation") and never claim a match with another house.
- **Product & Brand Disclaimer** (returns page + `custom_text_settings.json` + `js/pages/returns-shipping.js` + `admin-settings.js`): independent-label statement only - no "inspired", no scent-reference framing.
- **Canonical return terms (all pages must match):** 7-day window; exchange = two-way courier fee R120 x 2 = R240; refund = R120 return leg deducted, plus a possible 15% administrative fee; damaged/leaking bottle reported within 7 days with photos. **R120 is the standard courier rate for a return leg - the R85 is only our discounted delivery rate and must never be quoted as the return fee.** (The returns page carries a one-line note explaining the difference.)
- Policy page size is deliberate: `about.html` 5 blocks, `terms.html` 8 clauses, `privacy.html` 7 clauses. Keep them short and consistent with the returns page. `privacy.html` deliberately has **no** "Contact & Complaints" block and **no** Information Regulator contact details - the owner does not want that advertised; section 7 ("Your Rights & Contact") is where the policy ends.
- **Firestore/admin protection (needed 22 Sep 2026):** the admin Site Texts / Returns editors write back into `custom_text_settings.json` via `syncToGithub` and did revert the compliance copy once ("Trusted Clone Brand", `{brand}` comparison, "inspired by" disclaimer). `window.minaraComplianceOk()` (header.js) + `compliant()` (js/pages/returns-shipping.js) now reject stored text containing brand/comparison wording (brand placeholder, designer, clone, dupe, replica, counterfeit, knock-off, match, similarity index, olfactory profile, inspir, payfast), so the reviewed static copy always renders. If a data file is overwritten again: `git checkout HEAD -- custom_text_settings.json products.json` and push.
- The `.trust-item` trust banner markup no longer exists in any page, so that guard is dormant (harmless).
## Build / Deploy
...
- **Pending data mismatch** (from the outage): Firestore `products/profile-a-warm-day`.name = `A Warm Day` (test rename saved 2026-09-14). It is now mirrored into `products.json` by the fallback sync — rename it in the admin (or change it in `products.json` + push) if that was only a test.
...
- `products.json` — catalog. Fields: id, name, nameShort, price (ZAR), retailPrice (comparable RRP), stock, image/image_thumb/galleryImages, customisations[], isBundle, bundleSize, sizes, status.
...
- `js/home/home-products.js` — homepage best-seller cards (`buildCardHtml`); renders the `Comparable R<retail>` anchor inline next to the name.
...
- Card pattern: title -> type (ONLY on the bundle: `50ml extraits · you choose the scents`; singles render NO type line — `typeText` is `''`/hidden) -> inspired-by line (`INSPIRED BY` + the brand-free descriptor taken from the product's `name`) -> price block -> button. The product title/short name stays the fragrance's own theme (`nameShort`); the descriptor owns the `name` field only. For inspired products the retail anchor renders INLINE to the right of the name (`.hp-retail-price`) — the compliance copy ships no `retailPrice` at all.
...
## Gotchas
- `custom_text_settings.json` default titles differ from `admin-settings.js` defaults (e.g. "EXTRAIT DE PARFUM CONCENTRATION" vs "HAND-BLENDED IN SOUTH AFRICA"); `loadCustomTextSettings()` has rescue logic that may normalise/wipe stored edits.
...
- Contact page: WhatsApp label is now "MESSAGE ON WHATSAPP:" (removed "MESSAGES ONLY - NO CALLS"); the intro copy now says "crafting original extraits de parfum".
...
- Homepage review card: added `#homeTestimonialsInspired` line ("INSPIRED BY <descriptor>") resolved from the product's `name` in `home-reviews.js` (`inspiredMap`), and the card is slimmer/more subtle (`.home-testimonials-inner` 520px, card 500px/#fafafa/1px #ececec border, smaller quote & type, `.home-testimonials-inspired` 8.5px grey). Cache-busters bumped to `20260905` for `home.css`, `home-reviews.js`, `catalog-grid.js`, `product-core.js`, `product-customisation.js`.
