...
- Desktop top-header height is now **54px** (was 45px) via a `@media (min-width: 901px)` override appended to each stylesheet/page; base header stays **45px on mobile**. Homepage `.hero` (home.css) and checkout fixed offsets (`checkout.html` `.checkout-container`/`.summary-column`) were synced to 54px on desktop. Mobile home-carousel card image uses `aspect-ratio: 4/5` (portrait-ish; was 3/4 then 1/1) so the wider 60% card keeps the ADD TO BAG button in view without a big height jump. `.hp-inspired`/`.product-inspired` got 6px `margin-top` so the `INSPIRED BY` line sits lower. The black `.second-products-logo-wrap` logo above `MAKE THEM REMEMBER` (below the second hero image) is now ALWAYS shown, including on mobile (was `display:none` there).
